...
namespace Kamgam.HitMe
{
    ...
    public class PhysicalCharacterController : MonoBehaviour
    {
        ...
        protected void Update()
        {
            ...
            if (!isJumping)
            {
                targetPos = Jump.FindTarget(rb.linearVelocity * 1.3f);
        ...
        protected void FixedUpdate()
        {
            ...
            if (!isJumping)
            {
                ...
                var velocity = Quaternion.Euler(0f, rotationDelta * speedSign, 0f) * rb.linearVelocity;

                // Fast slow down
                if (Mathf.Abs(_speedLimit) > 0.001f)
                    rb.linearVelocity = velocity;
                else
                    rb.linearVelocity = new Vector3(velocity.x * 0.96f, velocity.y, velocity.z * 0.96f);
            }
        }
